...
url: https://docs.lammps.org/python.html
---
...
simulation run.  See the variable command doc page for
...
Use Python with LAMMPS section of the
...
to a python style variable with the return keyword.
...
its associated python style variable, as described
below.
...
itself using the LAMMPS Python module.  This
...
be used, as defined by the variable command.  The
...
this command creates an internal-style variable with
the specified name.
...
See the variable command doc page for full details on
...
is permissible to use a python-style variable in a
...
the function imports the  lammps  Python module.
...
The Use Python with LAMMPS page describes the syntax for how Python wraps
...
(e.g. fix setforce), then the Python function will
...
simulation run at each time step using the fix python/invoke command.
...
calculated by the compute ke command.  The 6
...
by input keyword to the python command.  For example,
...
function must be able to load the  lammps  Python module that wraps the LAMMPS library interface.
These are the same steps required to use Python by itself to wrap
LAMMPS.  Details on these steps are explained on the Python doc page.  Note that it is important that the stand-alone
...
- [fix python/invoke](fix_python_invoke.html)
