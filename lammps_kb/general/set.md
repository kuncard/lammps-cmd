...
read_restart or create_atoms commands, this command changes those assignments.
This can be useful for overriding the default values assigned by the
create_atoms command (e.g. charge = 0.0).  It
...
when they are output in dump files.  It can also be
...
The fix set command can be used with similar syntax
...
region.  See the group and region
...
nangletypes.  Or it can be a angle type label.  See the Howto
type labels doc page for the allowed syntax of
...
read_data command.  This keyword does NOT allow use
...
atom_style tri command.  The angular momentum
...
the specified value.  The value can be a numeric type from 1 to
nbondtypes.  Or it can be a bond type label.  See the Howto type
labels doc page for the allowed syntax of type
...
Keyword charge set the charge of all selected atoms.  The atom
style being used must support the use of atomic charge.
...
sphere) and its radius is non-zero, its mass is set from
...
If the atom has a shape attribute (see atom_style ellipsoid) and its 3 shape parameters are non-zero, then its mass
...
If the atom has a length attribute (see atom_style line) and its length is non-zero, then its mass is set from
...
must be finite-size spheres as defined by the atom_style sphere command.  The diameter of a particle can be set to 0.0,
...
a density, e.g. via the read_data command.
...
Howto type labels doc page for the allowed
...
the read_data command.  This keyword does NOT allow
...
DIELECTRIC package. Currently, only atom_style dielectric defines particles with this attribute. The value for the
...
dump command.  If a value of NULL is specified for any
...
diffusion coefficient is about to be measured via the compute
msd command.  Care should be taken not to reset the
...
must be line segments as defined by the atom_style line command.  If the specified value is non-zero the line
...
was defined with a density, e.g. via the read_data
command.

Keyword mass sets the mass of all selected particles.  The particles
must have a per-atom mass attribute, as defined by the
atom_style command.  See the  mass  command for
...
particles must be spheres as defined by the atom_style sphere command.  The angular velocity vector of the particles
...
atom_style command.  Note that particles defined
...
the atom_style command.  Random numbers are used
...
as described by the RHEO howto page.
...
particles must be ellipsoids as defined by the atom_style
ellipsoid command.  This command is used to define
...
temperature must be added using an instance of fix property/atom The values for the temperature must be positive.
...
atom_style line command.  Random numbers are used
in such a way that the orientation of a particular atom is the same,
...
triangles as defined by the atom_style tri
...
where ntypes was set by the create_box command or
the atom types field in the header of the data file read by the
read_data command.  When using a type label it must
have been defined previously.  See the Howto type labels doc page for the allowed syntax of type labels
...
Currently, only the atom_style peri command
...
\(\lambda\) is 0. The fast potential is used for an atom when its
switching parameter \(\lambda\) is 1. Both potentials are partially
used for \(\lambda\in(0,1)\).

Keywords i_name, d_name, i2_name, d2_name refer to custom
per-atom integer and floating-point vectors or arrays that have been
added via the fix property/atom command.
