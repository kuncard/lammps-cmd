---
id: thermo_modify
title: "thermo_modify command"
url: https://docs.lammps.org/thermo_modify.html
---
...
boundary or if it moves more than a box length outside
...
thermo keywords described by the page for the thermo_style command, all energy-related keywords are extensive, such
...
and computes in a thermo_style custom command, the
page for the individual fix or compute
...
the column number from the right (i.e., \(-1\) is the last
...
provided to the thermo_style command.
...
any of quantities the thermo_style command
...
must have been previously defined by the user via the compute command and it must be a style of compute that calculates a
pressure.  As described in the thermo_style
...
pressure compute defines its own
...
pxx,pyy,pzz,pxy,pxz,pyz keywords of the thermo_style command.  See the thermo_style
