---
id: thermo_style
title: "thermo_style command"
url: https://docs.lammps.org/thermo_style.html
---
...
corresponding to the list of keywords is determined by the units command for the simulation.  E.g. energies will be in energy
...
Style yaml is similar to style one but prints the output in YAML format which can be easily read by a variety of
...
Information about processing such YAML files is in the structured
data output howto.
...
to LAMMPS.  See the Modify page for details on the
...
of freedom.  Thus, using different compute commands
for calculating temperature, via the thermo_modify temp command, may yield different kinetic energies, since
...
also in time units, which is simply (step*dt) if the
timestep size has not changed and the timestep has not been reset.  If
the timestep has changed (e.g. via fix dt/reset)
...
been configured via the timer timeout command.  If the
...
triclinic (non-orthogonal).  See the Howto_triclinic doc page for a detailed explanation of orthogonal,
...
coordinate, as explained on the Howto_triclinic doc page.  If the thermo_modify
triclinic/general option is set then they are the
...
Howto triclinic page for a precise definition
...
symmetric pressure tensor for the system.  See the compute
pressure command doc page for details of how it is
calculated.

If the thermo_modify triclinic/general option
...
Howto_triclinic doc page, even if the
...
individual atom can be referenced in a equal-style variable and the variable referenced by thermo_style custom, as
...
compute command for details.  If the compute
...
norm option being used.

The f_ID and f_ID[I] and f_ID[I][J] keywords allow global values
calculated by a fix to be output.  As discussed on the fix doc page, fixes can calculate global, per-atom, local, and
per-grid values.  Only global values can be referenced by this
command.  However, per-atom fix values can be referenced for an
individual atom in a equal-style variable and the
...
when output, depending on the thermo_modify norm option being used.
...
variable command for details.  Note that variables
