...
id: rerun
title: "rerun command"
url: https://docs.lammps.org/rerun.html
---
...
for large systems.  See the page for the read_dump and dump commands which describe how to read
...
run command. It is set to no by default.
...
the read_dump page for details on the various
...
or fix npt are not invoked, since no time integration
...
displacements over time, you do not need to define a pair style, which may also mean neighbor lists will not need to be
calculated which saves time.  The comm_modify cutoff command can also be used to ensure ghost atoms are
...
as if the reset_timestep command were used.
...
fix ave/time page for details.  You must
