...
url: https://docs.lammps.org/processors.html
---
...
processes LAMMPS is running on.  For a 2d simulation,
Pz must equal 1.
...
number of MPI processes in this partition; see the -partition
command-line switch page for details.  Also note that you
can prefix the processors command with the partition command to easily specify different Px, Py, Pz values
...
subdivide the cores into NUMA domains.  Currently,
...
you have defined via the -partition command-line switch.
...
-partition command-line switch.  The universe and
original IDs will only be different if you used the -reorder
command-line switch to reorder the MPI processes
...
- [-reorder command-line switch](Run_options.html)
