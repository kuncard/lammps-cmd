...
balance command, which can adjust processor subdomain
sizes and shapes on-the-fly during a run.
...
an irregular-shaped geometry containing void regions, or hybrid
pair style simulations which combine pair styles with
...
Note that the processors command allows some
...
comm_style tiled to return the domain partitioning
...
(as specified by the neigh_modify command), then
...
breakdown, namely, for sections Pair, Bond,
...
the fix property/atom command.  This means
the values in the vector can be read as part of a data file with the
read_data command or specified with the set command.  These weights can also be output in a dump file, so this is a way to examine, debug, or visualize the
