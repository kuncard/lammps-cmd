---
id: neigh_modify
title: "neigh_modify command"
url: https://docs.lammps.org/neigh_modify.html
---
...
skin distance (specified in the neighbor command)
...
restart file is written, and on timesteps when a fix
requests reneighboring (e.g. fixes that create or delete atoms, such as
fix deposit or fix evaporate).  This ensures that all atoms are correctly assigned
to their owning subdomains before the operation is performed.  A
consequence is that enabling restart output can change
...
that are 1/2 the size of the maximum pair cutoff.  For neighbor
style multi, the bins are 1/2 the size of the collection
...
size of the bin used to sort atoms using the atom_modify command. If adjusted, it is recommended to keep the increase
