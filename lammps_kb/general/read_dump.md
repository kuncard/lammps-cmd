---
id: read_dump
title: "read_dump command"
url: https://docs.lammps.org/read_dump.html
---
...
the usual way, e.g. in a data file read in by a read_data
...
native LAMMPS dump files, written with a dump atom or
...
created with default settings by dump style xyz), that
number will be used as the atom type.  If the atom label is a string,
then a type map must be created using the labelmap command.  This map needs to associate each (numeric) atom type with
...
dump_modify sort command if the XYZ dump file was
written by LAMMPS.
...
dump_modify sort command if the dump file was
written by LAMMPS.
...
(default setting is yes).  See the reset_timestep
...
now have inconsistent image flags.  As discussed on the Errors common page this may or may not cause problems
