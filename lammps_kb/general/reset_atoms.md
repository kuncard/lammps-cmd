---
id: reset_atoms
title: "reset_atoms command"
url: https://docs.lammps.org/reset_atoms.html
---
...
processor have consecutive IDs, as the create_atoms command explains.
...
that are owned by other processors.  The comm_modify cutoff command can be used to correct this issue.  Or you can
...
compute fragment/atom command.  For each
...
Note
Same as explained for the compute fragment/atom command, molecules are identified using the
current bond topology.  This will not account for bonds broken by
the bond_style quartic command, because this
bond style does not perform a full update of the bond topology data
structures within LAMMPS.  In that case, using the delete_bonds
all bond 0 remove will permanently delete such
...
used is the one of compute fragment/atom.
...
molecular dynamics run with fix bond/react,
fix bond/create, or fix bond/break, all of which can change molecule topologies. It can
also be useful after molecules have been deleted with the
delete_atoms command or after a simulation which
has lost molecules, e.g. via the fix evaporate
command.
...
compute chunk/atom molecule or fix
rigid molecule.
...
bond style does not perform a full update of the bond topology data
structures within LAMMPS.  In that case, using the delete_bonds
all bond 0 remove will permanently delete such broken
...
- [compute fragment/atom](compute_cluster_atom.html)
