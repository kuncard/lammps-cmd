...
the Howto grid doc page for details.  The image
...
dump_modify thresh and region and refresh
commands can also alter what atoms are included.  Not all styles
support these options; see details on the dump_modify doc page.
...
timesteps dump snapshots are written.  See the dump_modify
pbc command if you wish to force coordinates to be
strictly inside the simulation box.

Note
Unless the dump_modify sort option is invoked,
...
running on a single processor, if the atom_modify sort option is on, which it is by default.  In this case
...
the dump_modify page for details.
...
style dump files can be read directly by VMD, a popular tool for visualizing
...
Likewise the OVITO visualization package,
...
Note that settings made via the dump_modify
...
text-based dump files which is controlled by the dump_modify
format command and its options.
...
rerun command.  See tools described on the Tools doc page for examples, including Pizza.py.
...
general triclinic.  See the Howto triclinic
...
triclinic page for a geometric description of
...
so that post-processing tools like OVITO,
...
cell IDs are included in the output via the compute
property/grid command, then the IDs will
...
using input from the compute property/local command with dump local, it is possible to
generate information on bonds, angles, etc. that can be cut and pasted
directly into a data file read by the read_data
command.
...
extended CFG format files, as used by the AtomEye visualization package.
...
use the dump_modify element command with
...
unwrap option of the dump_modify command allows
...
and colors. You can use the dump_modify types labels
option to replace numeric atom types with type labels.

Added in version 2Apr2025.

The extxyz style writes XYZ files compatible with the Extended XYZ (or
ExtXYZ) format as defined as defined in the libAtoms specification. Specifically, the following
information will be dumped:

Dump style extxyz requires either that a type label map for atoms types is defined or dump_modify element is used to
...
read, e.g. the ParaView tool.  For this
...
the filename.  In addition to per-atom data, thermo
data can be included in the yaml style dump file using the
dump_modify thermo yes. The data included in the
dump file uses the  thermo  tag and is otherwise identical to data
specified by the thermo_style command.
...
can be changed via the dump_modify first command,
...
dump_modify every command (not allowed for dcd
style).  The dump_modify every command also
...
time units of the units command command), the dump_modify
every/time command can be used.  This can be useful
when the timestep size varies during a simulation run, e.g. by use of
the fix dt/reset command.
...
when the dump command is invoked and closed when an undump command is used or when LAMMPS exits.  For the dcd and
...
dump_modify pad command can be used to ensure all
...
the binary2txt tool) or write your own code to read
...
supported compression format suffix, the dump file (or
files, if  *  or  %  is also used) is written in compressed format.  A
compressed dump file will be about \(3\times\) smaller than the text
...
suffix. See the dump_modify page for details on
...
B, C can be specified.  See the Howto triclinic doc page for a detailed description of general
triclinic boxes.
...
Howto_triclinic doc page, internally, LAMMPS
...
For dump output, if the dump_modify triclinic/general command is used, the box description and per-atom
...
output for unscaled atom coords, via the dump_modify scaled no setting. For style custom, this alters output for
...
Note
For style custom, the dump_modify thresh
...
atom type label.  Element is typically the
chemical name of an element, which you must assign to each type via the
dump_modify element command.  More generally, it can
...
been defined previously in the input script.  See the compute command for details.  There are computes for calculating the
...
defined previously in the input script.  The fix ave/atom command is one that calculates per-atom quantities.
Since it can time-average per-atom quantities produced by any
compute, fix, or atom-style variable, this allows those time-averaged results to be written to a
dump file.
...
been added via the fix property/atom
...
See the Modify page for information on how to add
...
custom command, and per-atom quantities can be output
...
See the Build extras page for details.
While a dump command is active (i.e., has not been stopped by using
the undump command), no commands may be used that will
change the timestep (e.g., reset_timestep).
...
- [dump atom/adios](dump_adios.html)
- [dump custom/adios](dump_adios.html)
- [dump cfg/uef](dump_cfg_uef.html)
- [dump h5md](dump_h5md.html)
- [dump image](dump_image.html)
- [dump molfile](dump_molfile.html)
- [dump netcdf](dump_netcdf.html)
- [dump netcdf/mpiio](dump_netcdf.html)
- [dump_modify](dump_modify.html)
- [undump](undump.html)
- [write_dump](write_dump.html)
