---
id: create_atoms
title: "create_atoms command"
url: https://docs.lammps.org/create_atoms.html
---
...
explicitly via a read_data or read_restart command.
...
using the lattice command, unless you specify the
...
read by the molecule command, and those values are
...
by the boundary command.  However, you can first
use the change_box command to temporarily
...
See the change_box doc page for an example of
...
and also consistent with the region volume.  See the region command for details.  Note that a region can be specified so
...
edge vectors.  See the Howto_triclinic doc
page for a detailed explanation of orthogonal, restricted triclinic,
and general triclinic simulation boxes.  As with the create_box command, the lattice command used by
this command must be of style custom and use its triclinic/general
option.  The a1, *a2, a3 settings of the lattice
command define the edge vectors of a unit cell of the general
triclinic lattice. The create_box command creates
...
the box created by the create_box command as
...
invoked.  The Howto_triclinic doc page also
...
volume.  See the region command for details.  Note
...
interleaving the create_atoms command with lattice
...
For the mesh style, a file with a triangle mesh in STL format is read and one or
...
stl_bin2txt tool.  The use of the units box option
...
on mesh triangles using an approach by (Roberts).
...
particles using neigh_modify exclude.
...
atom style template command, they must use
...
See the lattice command for specifics on how basis
...
group.  If the group does not yet exist it will be
...
an equal-style variable that should evaluate to a
...
When an atom is about to be created, its \((x,y,z)\) coordinates
...
size (see atom_style sphere for example) Doverlap
...
distance units as defined by the units command (e.g.,
\(\AA\) for units = real or metal.  A lattice value means
...
are defined by the chosen atom style.  See the
atom style command for more details.  See the
set and velocity commands for info on
...
Restrictions 
An atom_style must be previously defined to use this
command.
A rotation vector specified for a single molecule must be in
the z-direction for a 2d model.
For molecule templates that are created from multiple
