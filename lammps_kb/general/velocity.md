...
url: https://docs.lammps.org/velocity.html
---
...
rigid bodies, as defined by the fix rigid or fix rigid/small commands.  In other words, zero linear will set the
...
back to the atom velocities.  See the Howto thermostat page for more discussion of temperature computes
...
will not be the case if atoms were created using the create_atoms command, since atom IDs will likely be assigned to atoms
differently.
...
rigid bodies or SHAKE constraints may not have the desired outcome for two reasons.  First,
...
perform a run 0 command, which ensures all DOFs are
