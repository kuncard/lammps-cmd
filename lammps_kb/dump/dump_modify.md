---
id: dump_modify
title: "dump_modify command"
url: https://docs.lammps.org/dump_modify.html
---
...
various dump styles, including the dump image and
dump movie styles.
...
equal-style variable, which should be specified as
...
could easily be added as options for equal-style variables.  Also see the next() function, which allows use of a
...
elapsed simulation time, i.e. in time units of the units command, and specifies Delta for the interval between
snapshots.  This can be useful when the timestep size varies during a
simulation run, e.g. by use of the fix dt/reset
...
(i.e. in time units of the units command).  This can be
useful when the timestep size varies during a simulation run (e.g., by
use of the fix dt/reset command).  The default is
...
0.0.  Or it can be an equal-style variable, which
...
time can be used in an equal-style variables since
...
frequency specified in the dump command or
dump_modify every command, including timestep 0.
It will also always occur if the current simulation time is a multiple
of Delta, the time interval specified in the dump_modify
every/time command.
...
Note that if the argument to the dump_modify every dump_modify every/time commands is
...
data produced by pair tracker or
bpm bond styles which may not require the
...
components of the displacement.  The dump_modify thresh
command will cause only atoms that have displaced more than
\(0.6~\AA\) to be output on a given snapshot (assuming
...
stored by compute displace/atom.  So the
...
shown as part of the compute displace/atom
...
\(\AA\) and fs used by real units are 0.1 nm and
...
is met.  The condition is computed by an equal-style variable, which should be specified as v_name, where name is the
...
proceeds as usual.  Note that equal-style variable
...
parallel.  This is the case even in serial if the atom_modify sort option is turned on, which it is by default, to improve
...
It triggers writing of thermo information to the dump
...
values as specified by the thermo_style command.
...
dump custom command.  For example, you can output the
...
See the variable command page for details.
...
the dump command for details of how this changes the
...
type labels are printed for atom types.
...
Howto_triclinic doc page for a detailed
...
dump doc page for a full list of which dump attributes
this affects.
...
The COMPRESS package offers both GZ and Zstd
...
compression_level keyword. File names with these styles have to
end in either .gz or .zst.

GZ supports compression levels from \(-1\) (default), 0 (no compression),
and 1 to 9, 9 being the best compression. The COMPRESS /gz styles use 9
...
default and it can be disabled with the checksum keyword.

The VTK package offers writing dump files in VTK file
formats that can be read by a variety of
...
vtk style dumps the dump_modify command supports the
