---
id: compute_pair_local
title: "compute pair/local command"
url: https://docs.lammps.org/compute_pair_local.html
---
...
Changed in version 12Jun2025: The sign of dx, dy, dz is no longer determined by the value of
...
vector \(\vec{x_i} - \vec{x_j}\) between the pair of atoms.  The
order of the atoms is determined by the neighbor list and the respective
atom-IDs can be output with compute property/local.
...
granular pair styles, these values only include the
...
An example of ones that do are the granular pair styles
...
The value dist, dx, dy and dz will be in distance units.
The value eng will be in energy units.
The values force, fx, fy, and fz will be in force units.
...
compute property/local command can be combined
