---
id: compute_orientorder_atom
title: "compute orientorder/atom command"
url: https://docs.lammps.org/compute_orientorder_atom.html
---
...
\[\begin{split}\bar{Y}_{\ell m} = & \frac{1}{nnn}\sum_{j = 1}^{nnn} Y_{\ell m}\bigl( \theta( \mathbf{r}_{ij} ), \phi( \mathbf{r}_{ij} ) \bigr) \\
Q_\ell  = & \sqrt{\frac{4 \pi}{2 \ell  + 1} \sum_{m = -\ell }^{m = \ell } \bar{Y}_{\ell m} \bar{Y}^*_{\ell m}}\end{split}\]
...
compute coord_atom to calculate the ten Wolde s
...
- [compute hexorder/atom](compute_hexorder_atom.html)
