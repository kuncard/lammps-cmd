---
id: compute_property_atom
title: "compute property/atom command"
url: https://docs.lammps.org/compute_property_atom.html
---
...
apip_e_fast,apip_e_precise = potential energies mixed by the adaptive-precision potential
history arg = fixID
  fixID = ID of a fix store/state command with history enabled
```
...
output commands that take computes as inputs.
See for example, the compute reduce, fix
ave/atom, fix ave/histo,
fix ave/chunk, and atom-style variable commands.

The list of possible attributes is essentially the same as that used
by the dump custom command, which describes their
meaning, with some additional quantities that are only defined for
certain atom styles.  The goal of this augmented
...
fix store/state command using its history
...
The associated fix store/state command
...
fix store/state doc page for more details.

The additional per-atom attributes only accessible via this command,
and not directly via the dump custom command, are as
follows.
...
If newton bond off is set, it will be tallied with both atom
\(I\) and atom \(J\).
...
See the set command for an explanation of the blockiness.
...
The accessible quantities from the APIP package
are explained in the doc pages of this package in detail.  In short:
apip_lambda is the switching parameter \(\lambda\in[0,1]\).  The
...
for an atom \(i\) with \(\lambda_i=1\) one does not need
\(E_i^\text{precise}\) and with \(\lambda_i=0\) one does
not need \(E_i^\text{fast}\).
...
\(\lambda\in[0,1]\), that is calculated from apip_lambda_input
...
The energy according to the fast and the precise potential are only
computed for the subset of atoms for which it is required.  I.e.,
for an atom \(i\) with \(\lambda_i=1\),
\(E_i^\text{precise}\) is not needed.  And with
\(\lambda_i=0\), \(E_i^\text{fast}\). is not needed.
...
- [fix store/state](fix_store_state.html)
