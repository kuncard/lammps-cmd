---
id: fix_deform_pressure
title: "fix deform/pressure command"
url: https://docs.lammps.org/fix_deform_pressure.html
---
...
fix press/berendsen. The target pressure
accepts either a constant numeric value or a LAMMPS variable.
...
discussed below. There is no proven method
...
A maximum limit can be applied using the max/rate
option. Akin to fix npt and nph, pressures in different
dimensions can be coupled using the couple option.
...
constant numeric value or a LAMMPS variable. Notably, this variable can be a function of time or other
components of the pressure tensor. By default, \(k\) has units of
1/(time * pressure) although this will change if the
normalize/pessure option is set as discussed below.  There is no proven method to choosing an
...
the applied strain using the max/rate option.
