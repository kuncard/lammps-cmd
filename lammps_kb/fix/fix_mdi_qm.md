---
id: fix_mdi_qm
title: "fix mdi/qm command"
url: https://docs.lammps.org/fix_mdi_qm.html
---
...
The server code must support use of the MDI Library as
explained below.
...
commands fix gcmc and fix atom/swap, as well as the compute born/matrix command.  The only requirement is that internally
...
for MDI.  See the Howto mdi page for more
...
LAMMPS acting as a QM surrogate.  See the Howto mdi
...
codes either via this fix or the fix mdi/qmmm
command.
...
setting is no then the default fix_modify energy
and fix_modify virial settings are also set to
...
E.g. if it used inside a loop which also uses the clear
...
the mdi connect and exit command to one-time
...
accept/reject criterion.  The fix gcmc and fix
atom/swap commands can be used in this manner.
...
minimization includes fix box relax to
...
produce training data for a machine learning potential.  A dump
custom command could be invoked every 1000 steps to dump the
...
systems.  See the Howto multiple doc page for
...
read_data commands.  Or, for example, by using the
lattice , create_atoms,
delete_atoms, and/or displace_atoms
random commands to generate a series of different
systems.  At the end of the loop perform run 0 and
write_dump commands to invoke the QM code and
...
- [mdi plugin](mdi.html)
- [mdi engine](mdi.html)
- [fix mdi/qmmm](fix_mdi_qmmm.html)
