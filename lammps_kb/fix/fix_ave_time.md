...
following two fix ave/time commands are equivalent, since the compute rdf command creates, in this case, a global array with three
...
not in your input script, but by thermodynamic output or other fixes such as fix nvt or
fix temp/rescale.  See the doc pages for
...
conforms to the YAML standard which allows
easy import that data into tools and scripts that support reading YAML
files. The structured data Howto contains
