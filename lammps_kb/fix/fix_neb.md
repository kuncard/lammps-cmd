---
id: fix_neb
title: "fix neb command"
url: https://docs.lammps.org/fix_neb.html
---
...
where RD is the  reaction coordinate  see neb section,
...
respectively. \(ED_{ideal}\) is the corresponding cumulative sum of
...
along, or near to, NEB pathways such as fix_pafi.
...
resolution is poor.  I.e. when few replicas are used; see (Maras) for details.
...
1 when the angle \(R_{i-1} R_i R_{i+1}\) is acute.
\(F(R_{i-1}, R_i, R_{i+1})\) is defined in (Jonsson).
...
If \(E_{Target} > E\), the interatomic force \(F_i\) for the
