---
id: fix_bond_react
title: "fix bond/react command"
url: https://docs.lammps.org/fix_bond_react.html
---
...
Fix bond/react does not use quantum mechanical (e.g., fix qmmm) or
pairwise bond-order potential (e.g., Tersoff or
AIREBO) methods to
...
integrator such as fix nvt, fix npt, or
fix nve, as shown in the second example
...
reset_atoms mol command is invoked after a reaction
occurs, to ensure that molecule IDs are consistent with the new bond
topology.  The group-ID used for reset_atoms mol is
...
specified with an equal-style variable. Reaction
...
reaction is initiated. The output file follows the JSON dump
molecules format, with one extra key added to each
...
variable, whose value is rounded up to the nearest
integer.
...
equal-style variables. For example, these reaction cutoffs
...
command page. It is highly recommended to use Type labels
...
To use wildcard atoms, a specific type label
...
molecule fragment defined in the post-reaction molecule template, and only atoms in this fragment are used for the
...
Refer to the dihedral style documentation for
...
molecule command for the pre-reaction template.
...
molecule command for the pre-reaction template. For
...
formula that can be read by the variable command,
...
the variable command after converting each
...
1.0, and can be specified with an equal-style variable.
...
- [fix bond/create](fix_bond_create.html)
