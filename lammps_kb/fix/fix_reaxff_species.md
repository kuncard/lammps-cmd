...
title: "fix reaxff/species command"
url: https://docs.lammps.org/fix_reaxff_species.html
---
...
ReaxFF pair_coeff command and the ReaxFF force
field file.
...
from the system (Gissinger).  Criteria for deletion can
...
an equal-style variable. When using the
...
entire molecule when it is partially inside a region, as
...
deleted molecules in the style of the JSON molecule file,
...
objects in  timesteps  contains two keys,  timestep  and  molecules .  The
 molecules  key is an array of LAMMPS molecule JSON
...
corresponding to elements using type labels.
...
Restrictions 
The  fix reaxff/species  requires that pair_style reaxff is used.
