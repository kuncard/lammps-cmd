---
id: fix_propel_self
title: "fix propel/self command"
url: https://docs.lammps.org/fix_propel_self.html
---
...
\(f_P \sum_i <e_i . r_i>/(d V)\), where \(r_i\) is the
unwrapped coordinate of particle i in the case of periodic
boundary conditions. See (Winkler) for a
...
(Use fix_modify as described below
to turn off the virial contribution of this fix). Again,
see (Winkler) for discussion of why this
is the case.
...
compute pressure command (turning
...
also enabled.  See the Build package page for
more info.

## Related Commands

- [fix align/self](fix_align_self.html)
