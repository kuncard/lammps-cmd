---
id: pair_hybrid
title: "pair_style hybrid command"
...
pair_coeff command.  The major difference between
...
When trying to combine EAM or MEAM potentials, there is a large
error in the embedding term, since it is computed separately for
each sub-style only.
When trying to combine many-body potentials like Stillinger-Weber,
...
atom elements).
When one of the sub-styles uses charge-equilibration (= QEq; like
...
or  PairIJ Coeffs  section of a data file via the read_data command,
...
read_data command, or by mixing as described below.
...
sub-style via the pair_coeff command as in the
examples above, or in the data file read by the read_data, or by mixing as described below.  Also all sub-styles
must be used at least once in a pair_coeff command.
...
written out using the write_coeff command and
...
to some sub-style and use the neigh_modify exclude type
...
global setting defined the special_bonds command.
However, different weights can be assigned to different hybrid
sub-styles via the pair_modify special command.
...
See the pair_modify page for details on
...
pair command.  Note that in the case of pair style
