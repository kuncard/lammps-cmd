---
id: kspace_style
title: "kspace_style command"
url: https://docs.lammps.org/kspace_style.html
---
...
(Liang2025), a fast Ewald-summation approach that can reduce
...
per dimension at high accuracy (\(\approx 8\times\) in 3D). The residual
...
upsampling, PSWFs require fewer neighboring grid points (e.g., \(\approx 8\) vs \(\approx 12\)
...
run style, e.g., verlet/split/rk.  An error is
...
(Dandurand) and works cited within.
...
Build settings doc page. MSM does not
...
method implemented in fix electrode/*.  The styles
...
library which provides a variety of solver
methods which can be used with LAMMPS.  The paper by (Sutman) gives an overview of ScaFaCoS.
...
Build settings doc page for how to select a
...
default; the kspace_modify diff ad option is not
supported together with a triclinic box).  The kspace_modify slab correction is supported for a triclinic box only
...
reconstruct the M-site, see the TIP4P howto.
...
that package and the KSPACE package.  See the Build package page for more info.
...
the SCAFACOS package.  See the Build package
...
- [kspace_modify](kspace_modify.html)
- [pair_style lj/cut/coul/esp](pair_lj_cut_coul.html)
- [pair_style lj/cut/coul/long](pair_lj_cut_coul.html)
- [pair_style lj/charmm/coul/long](pair_charmm.html)
- [pair_style lj/long/coul/long](pair_lj_long.html)
- [pair_style buck/coul/long](pair_buck.html)
