...
parameters, see the long-range dispersion howto
discussion.
...
part of pppm/disp. As shown in (Isele-Holder),
...
as described in (Isele-Holder).  The none
...
boundary setting of  boundary p p f .  This is done
...
automatic choice is discussed in (Gao2025) and
(Gan2025).  For related error estimates and
parameter-selection guidance in the more general dielectric-confined
setting, see (Gao2025). The auto setting is
currently supported by kspace_style ewald, pppm,
...
(non-orthogonal) simulation cells for kspace_style
...
interactions (Klapp) in kspace_style ewald/disp, ewald/dipole, and pppm/dipole.
This dipole slab correction is currently limited to orthogonal
(non-triclinic) simulation cells.

Changed in version 4Jul2026: The slab correction with a volfactor value now supports triclinic
...
or by using the fix efield command.
